...
_serversteps                = {}
_serverdata                 = {}
defaultKeyboard             = ['Add', 'Remove', 'List'], ['Filters', 'Location'],['Pause', 'Resume','Help']
filterKeyboard             = ['Distance', 'Perfection'], ['Filter type'],['Cancel']
pokemonList = {
    "001": "Bulbasaur",
    "002": "Ivysaur",
    "003": "Venusaur",
...
def setfilters(bot, update):
    logger = logging.getLogger('poke' + "." + 'filters')
    logger.debug("Entering filters")
    
    chat_id = update.message.chat_id
    uname = update.message.chat.username
    
    custom_keyboard         = filterKeyboard
    reply_markup            = telegram.ReplyKeyboardMarkup(custom_keyboard,one_time_keyboard=True)
    _listener[uname] = 'filters'
    logger.debug("Set listener of {} to: {}".format(uname,_listener[uname] if uname in _listener else "?"))
    
    bot.sendMessage(update.message.chat_id, text='Select filter to change.',reply_markup=reply_markup)
...
def addpokemon(bot, update,args):
    ...
    if args is not None and len(args) > 0:
        ...
        poke = args[0]
        print "input:",poke
        print "Title:",poke.title()
        
        if poke in pokemonList.values() or poke.title() in pokemonList.values():
            
            if not poke in pokemonList.values(): poke = poke.title()
...
def removepokemon(bot, update,args):
    ...
    if args is not None and len(args) > 0:
        ...
        poke = args[0]
        if poke in selectedList or poke.title() in selectedList: 
            if poke not in selectedList: poke = poke.title()
...
def setperfection(bot, update,args):
    logger = logging.getLogger('poke' + "." + 'setperfection')
    
    chat_id = update.message.chat_id
    uname = update.message.chat.username
    logger.debug("Entering set perfection")
    ...
    if args is not None and len(args) > 0:
        ...
        users.update({'chat_id': chat_id},{"$set":{'min_iv': min_iv}})
        
        custom_keyboard         = defaultKeyboard
        reply_markup            = telegram.ReplyKeyboardMarkup(custom_keyboard,one_time_keyboard=True)
        bot.sendMessage(chat_id, text="Set minimum iv-value to {} %".format(min_iv),reply_markup=reply_markup)
        _listener[uname] = None
        return
    else:
        bot.sendMessage(chat_id, text="Notify of pokemons only above a certain value? Enter the value as percentage (default = 67)")
        _listener[uname] = 'setperfection'  

def setfiltertype(bot, update,args):
    logger = logging.getLogger('poke' + "." + 'setfiltertype')
    
    chat_id = update.message.chat_id
    uname = update.message.chat.username
    logger.debug("Entering set filter type")
    ...
    if args is not None and len(args) > 0:
        ...
        users.update({'chat_id': chat_id},{"$set":{'filter_type': filter_type}})
        
        custom_keyboard         = defaultKeyboard
        reply_markup            = telegram.ReplyKeyboardMarkup(custom_keyboard,one_time_keyboard=True)
        bot.sendMessage(chat_id, text="Set filter type to {}".format(filter_type),reply_markup=reply_markup)
        _listener[uname] = None
        return
    else:
        bot.sendMessage(chat_id, text="Enter filter type: 0 = filters off, 1 = full filter, 2 = silent filter (default = 2)")
        _listener[uname] = 'setfiltertype'  
...
def cli_noncommand(bot, update):
    ...
    if text == 'Cancel' or text == 'cancel' or text == 'home' or text == 'Home':
    ...
    elif text == 'Add' or text == 'add':
        addpokemon(bot, update, [])
    elif text == 'Remove' or text == 'remove':
    ...
    elif text == 'Distance' or text == 'distance':
       setdistance(bot, update,[])
    elif text == 'Perfection' or text == 'perfection':
       setperfection(bot, update,[])
        ...
        resume(bot, update)
    elif text == 'Filters' or text == 'filters':
        setfilters(bot, update)
    elif text == 'Filter type' or text == 'filter type':
        setfiltertype(bot, update, [])
        ...
        elif uname in _listener and _listener[uname] == "setperfection":
            setperfection(bot, update, [text])
        elif uname in _listener and _listener[uname] == "setfiltertype":
            setfiltertype(bot, update, [text])
        elif uname in _listener and _listener[uname] == 'filter':
            if text is not None and len(text) > 0:
                if text == 'Distance' or text == 'distance':
                    setdistance(bot, update,[])
                elif text == 'Perfection' or text == 'perfection':
                    setperfection(bot, update,[])
        else:
...
def main():
    ...
    dp.add_handler(CommandHandler("setperfection", setperfection,pass_args=True))
    dp.add_handler(CommandHandler("setfiltertype", setfiltertype,pass_args=True))
